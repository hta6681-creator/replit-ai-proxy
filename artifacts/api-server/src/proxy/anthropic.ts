...
import { makeLoggedFetch } from "./clients.js";

const loggedFetch = makeLoggedFetch("anthropic-native");
...
async function handleNativeNonStream(
  ...
  try {
    const upstream = await withRateLimit(async () => {
      const resp = await loggedFetch(upstreamUrl, {
        method: "POST",
        headers,
...
async function handleNativeStream(
  ...
  try {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      upstream = await loggedFetch(upstreamUrl, {
        method: "POST",
        headers,
        body: bodyStr,
        signal: controller.signal,
